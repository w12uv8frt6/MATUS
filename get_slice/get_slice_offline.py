...
def get_subGraph(list_DDGvariable, pdgGraph_temp, len_edges, path_ast):
    ...
    for DDGvariable_temp in list_DDGvariable:
        ...
        subgraph_edges = remove_duplicates(subgraph_edges) #去重
...
def generate_subGraph_data(path_pdgdot, path_ast, path_c, len_edges, list_top_statements, model):
    ...
    if len(list_top_statements) > 0:
        for dict_top_statement in list_top_statements:
            ...
            with open(path_c, 'r', encoding = encoding_message['encoding']) as f:
                lines_c = f.readlines()
            f.close()
            statement = lines_c[int(statement_linenumber) - 1].strip()
            ...
            if len(common_elements_list) > 0:
                ...
                if len(common_elements_list) <= 2:
                    ...
                    for index_for_variableMask, variable_temp in enumerate(common_elements_list):
                        ...
                        if len(list_statement_concatenate_with_mask) > 0:
                            for index, statement_concatenate_with_mask in enumerate(list_statement_concatenate_with_mask):
                                ...
                                saveembedding_path_temp = ""
                                if path_c.endswith(".c"):
                                    saveembedding_path_temp = path_c[:-2] + "____linenumber" + str(statement_linenumber) + "____index_for_variableMask_" + str(index_for_variableMask) + "____maskEmbedding_" + str(index) + ".pth" 
                                    torch.save(norm_mask_embedding_reshaped_2, saveembedding_path_temp) 

                                dict_temp = dict()
                                dict_temp["index_for_variableMask"] = index_for_variableMask
                    ...
                    if len(list_of_dict_variable) > 0:
                        list_of_dict_variable_sorted = sorted(list_of_dict_variable, key=lambda x: x["index"], reverse=False) 
                        index_temp = int(len(list_of_dict_variable_sorted)) 

                        top_list_of_dict_variable_sorted = list_of_dict_variable_sorted 
                        new_common_elements_list = [item["variable_temp"] for item in top_list_of_dict_variable_sorted]
                        new_common_elements_list = remove_duplicates(new_common_elements_list)
                        ...
                        if len(subGraph_withlinenumber) > 0:
                            if len(subGraph_withlinenumber_original) == 0:
                                if check_rootstatement_in_subgraph(subGraph_withlinenumber, statement_linenumber_start, statement_linenumber_stop):
                                    dict_temp_forEachVariable["variable_name"] = common_elements_temp.strip()
                                    dict_temp_forEachVariable["subGraph_withlinenumber"] = subGraph_withlinenumber
                                    list_subGraph_withlinenumber_in_statement.append(dict_temp_forEachVariable)
                            else:
                ...
                if len(list_subGraph_withlinenumber_in_statement) > 0:
                    ...
                    dict_temp["list_of_dict_variable"] = list_of_dict_variable 
                    list_top_statements_new.append(dict_temp)
    
    return list_top_statements_new


def main(result_jsonfilepath, result_jsonfilepath_new, model):
    ...
    for index, dict_target_function in enumerate(target_functions_list):
        ...
        len_edges = 1
        list_top_statements_new = generate_subGraph_data(path_pdgdot, path_ast, path_c, len_edges, dict_target_function["list_top_statements"], model)
...
if __name__ == "__main__":
    ...
    json_name_new = "get_slice-all.json" 
    ...
    with torch.no_grad():
        main(result_jsonfilepath, result_jsonfilepath_new, model)
